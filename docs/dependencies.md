...
loudly instead of silently: an entry that stops matching fails the run. The audit
job in ci.yml is no longer path-filtered either. It was gated on `**/Cargo.toml`
