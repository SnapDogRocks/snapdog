...
`TokioTimeGuard` (`IT-T03`), `EphemeralResource` pool (`IT-T02`), the deterministic app
harness (`build_test_app`/`spawn_zone_harness`: TempDir + persist-off + receivers-off, `IT-T04`), the golden helper (`IT-T06`),
...
- [x] `IT-T01` `testkit` scaffold: `tests/common/` + `test-util` feature exporting reusable doubles (`IT-DEC-08`). `status: done` · deps: — · **AC:** `cargo test` discovers `tests/common`; `--features test-util` compiles.
...
- [x] `IT-T04` `TempEnv` fixture (TempDir `state_dir`, pre-seeded `server_id`, `persist_path` control, `mdns.enabled=false`). `status: done` · deps: IT-T01 · **AC:** a deterministic per-test env — TempDir-backed state, no auto-save, no mDNS. **Done:** shipped as the `build_test_app` / `spawn_zone_harness` helpers (`tests/common`), not a standalone `TempEnv` struct: each backs `EqStore` with a `TempDir`, passes `persist_path=None` (no auto-save loop) and `start_receivers=false` (no RAOP bind / mDNS). A pre-seeded `server_id` proved unnecessary (no test asserts on it) and was omitted.
...
- [x] `IT-T10` In-process REST driver: `oneshot` on `Router` + mock `AppState` (captured `ZoneCommand`/`SnapcastCmd` mpsc + broadcast tap). `status: done` · deps: IT-T01 · **AC:** a GET returns 200 with no TCP socket; the suite **enumerates and asserts every mounted route group** (no hardcoded endpoint count).
- [x] `IT-T11` Zone endpoints contract (all): status+body+**exactly-one** command; boundaries (zone 0→404), seek XOR(→400), volume parse/clamp, repeat cycle, EQ band limits, cover placeholder etag. `status: done` · deps: IT-T10, IT-T03 · **AC:** every zone endpoint returns documented status+body and captures exactly one `ZoneCommand`; zone 0→404; seek both→400 / exactly-one→200; volume parse/clamp; repeat cycles Off→All→One; EQ >10 bands→400 + band-edit clears preset; `GET …/cover`→200 PNG + ETag `"snapdog-placeholder"`.
...
- [x] `IT-T20` All **7** notification variants emitted on the right mutation; serde `tag`/snake_case round-trip; **a compile-time exhaustiveness match over all 7 `Notification` variants** (catch silent add/rename, mirrors IT-T52). `status: done` · deps: IT-T10, IT-T03.
...
- [x] `IT-T53` `GroupVolumeMode.effective()` table tests (Absolute/Relative/Compressed + clamp + max_volume). `status: done` · deps: IT-T01.
